      ...
      <div className="flex flex-row gap-2 justify-between">
        <Link href={`/projects/${prevProject.slug}`}>
          <Button className="rounded-none group" variant={"secondary"}>
            <ArrowLeft className="size-4 group-hover:-translate-x-0.5 transition-transform" /> Prev{" "}
        ...
        <Link href={`/projects/${nextProject.slug}`}>
          <Button className="rounded-none group" variant={"secondary"}>
            ...
            Next <ArrowRight className="size-4 group-hover:translate-x-0.5 transition-transform" />
          </Button>
        </Link>
      </div>
      <Separator />
          ...
          <ul className="flex flex-wrap justify-center gap-2 mt-12">
            {metadata.tags?.map((tag: string) => (
              <Badge
                key={tag}
                variant="secondary"
                className="!border bg-background/50 p-2 shadow-xl"
              >
                {tag}
              </Badge>
            ))}
        ...
        {metadata.links && (
          <div className="flex w-full flex-wrap gap-0  items-center bg-background/50">
            {Object.entries(metadata.links).map(([label, url]) =>
              url ? (
                <Link key={label} href={url} target="_blank" className="flex flex-1">
                  <Button
                    variant="secondary"
