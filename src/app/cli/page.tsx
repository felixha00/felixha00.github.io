      ...
      <div className="mx-auto grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="relative overflow-hidden bg-gradient-to-b from-slate-900 to-slate-950 border-slate-800 text-slate-100 shadow-xl">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            ...
            <div className="flex items-center gap-2">
              <Badge
                variant="secondary"
                className="bg-emerald-500/10 text-emerald-300 border-emerald-700"
              >
                interactive
              </Badge>
              <Button
          ...
          <CardContent className="p-0">
            ...
            <div className="border-t border-slate-800">
              <div className="flex items-center gap-3 p-3 font-mono text-sm">
                ...
                <Button size="sm" className="gap-1" onClick={() => run(value || "help")}>
                  Run
        ...
        <Card className="bg-card border-muted shadow-xl">
          ...
          <CardContent className="p-4">
            <Tabs
              defaultValue={route.section || "all"}
              value={route.section || "all"}
              onValueChange={(v) =>
                setRoute({
                  page: "works",
                  section: v === "all" ? undefined : v,
                })
              }
            >
...
// --- Render helpers -----------------------------------------------------------
function renderHelp() {
  const list = ALL_COMMANDS.map(
    (c) =>
      `• <span class='text-cyan-300'>${c.cmd}</span> — <span class='text-slate-300'>${c.desc}</span>`,
  ).join("<br/>");
  return (
    `<div class='space-y-2'>` +
