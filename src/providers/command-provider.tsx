  ...
  const runCommand = async (command: string) => {
    ...
    switch (cmd.toLowerCase()) {
      case "theme":
        ...
        output = (
          <p>
            Set <span className="text-ansi-magenta">theme</span> to{" "}
            <span className="text-ansi-green">{args[0]}</span>
          </p>
        );
        break;
      case "whoami":
                  ...
                  {Object.entries(allDeps).length > 0
                    ? Object.entries(allDeps).map(([name, version]) => (
                        <div key={name} className="text-[var(--tw-prose-body)] font-normal">
                          <span className="text-green-500">{name}</span>/{version as string} stable
                          [installed]
                        </div>
                      ))
      ...
      default:
        try {
          ...
          output = <p>{command}: command not found</p>;
        }
    }
