    ...
    >
      ...
      <Link href={`${baseRoute}/${item.slug}`}>
        <motion.div
          ...
          className="relative p-6 bg-background bg-cover hover-brightness border "
        >
          {/* bg-[url('/img/default-bg.webp')] */}
          <motion.div
            variants={itemVariants}
            className="absolute right-6 bottom-0 z-[1] mb-[-0.5rem]"
          >
            {item.projectType && (
          ...
          <div className="aspect-video bg-background relative">
            <Image
              src={item.image || "/img/default-bg.webp"}
              alt={item.title}
              fill
      ...
      <CardContent className="flex flex-col grow gap-2 p-6 pt-2">
        ...
        <motion.time
          variants={itemVariants}
          className="text-xs font-mono tracking-tight text-muted-foreground"
        >
        ...
        {item.stack && item.stack.length > 0 && (
          <motion.div variants={itemVariants} className="flex flex-wrap gap-1">
            {item.stack.map((tech: string) => {
              ...
              const iconNode = stackItem?.icon || <span className="text-xs font-mono">{tech}</span>;
              ...
              return (
                <Tooltip key={tech}>
                  ...
                  <TooltipContent>{stackName} ↗</TooltipContent>
                </Tooltip>
              );
            })}
          </motion.div>
