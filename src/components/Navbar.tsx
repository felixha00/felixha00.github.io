...
const Navbar = () => {
    ...
    return (
        <nav className='z-50 h-16 fixed top-0 left-0 right-0 p-4 flex items-center justify-between bg-linear-to-b from-background to-100% to-transparent'>
            <div /*ref={containerRef}*/ className='flex items-center z-100 mix-blend-difference'>
                {/* Name — commented out, restore with scroll crossfade logic above
                <motion.h1
                    animate={{
                        y: showLogo ? -15 : 0,
                    ...
                    className={`col-start-1 row-start-1 w-fit text-2xl font-extrabold opacity-100 tracking-tight -translate-y-1 mix-blend-difference text-white font-display ${showLogo ? 'pointer-events-none' : 'pointer-events-auto'}`}
                >
